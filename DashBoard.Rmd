...
Consulta_historial_salarios <- "
SELECT TOP 10
    e.nombre AS Empleado,
    hs.salario_anterior AS Salario_Anterior,
    hs.salario_nuevo - hs.salario_anterior AS Aumento_Salario,
    hs.fecha_actualizacion AS Fecha_Actualizacion
FROM 
    empleados e
INNER JOIN 
    historial_salarios hs ON e.id_empleado = hs.id_empleado
ORDER BY 
    Aumento_Salario DESC;
"

historial_salarios <- dbGetQuery(con, Consulta_historial_salarios)

historial_salarios$Fecha_Actualizacion <- as.Date(historial_salarios$Fecha_Actualizacion)

# ventas por ciudad

consulta_ventas_ciudad <- "SELECT c.ciudad, COUNT(v.id_venta) AS cantidad_ventas
FROM clientes c
INNER JOIN ventas v ON c.id_cliente = v.id_cliente
GROUP BY c.ciudad
ORDER BY cantidad_ventas DESC;"

ventas_ciudad <- dbGetQuery(con, consulta_ventas_ciudad)

#ventas por empleado

consulta_ventas_empleado <- "SELECT e.nombre AS empleado, COUNT(v.id_venta) AS cantidad_ventas
FROM empleados e
INNER JOIN detalles_venta dv ON e.id_empleado = dv.id_empleado
INNER JOIN ventas v ON dv.id_venta = v.id_venta
GROUP BY e.nombre
ORDER BY cantidad_ventas DESC;"

ventas_empleado <- dbGetQuery(con, consulta_ventas_empleado)
...
### ventas por empleado
```{r}
custom_colors <- viridis::turbo(n = 35)

ventas_empleado %>%
  head(35) %>%
  group_by(empleado) %>%
  hchart('pie',hcaes(x = empleado, y= cantidad_ventas, color = custom_colors )) %>%
  hc_add_theme(hc_theme_538()) %>%
  hc_tooltip(pointFormat = '<b>Ventas: </b>{point.y}<br>') %>%
  hc_title(text = 'Empleados con mas ventas', style = list(fontSize = '45px', fontWeight = 'bold')) 
...
empleados_mas_altos_sueldo %>%
  ...
  hc_title(text = 'Empleados mejor pagados', style = list(fontSize = '15px', fontWeight = 'bold')) %>%
  hc_subtitle(text = 'Ordenados tomando en cuenta su salario actual', style = list(fontSize = '12'))
...
```{r}
custom_colors <- viridis::turbo(n = 105)

ventas_ciudad %>%
  head(105)%>%
  hchart('scatter', hcaes(x = ciudad, y = cantidad_ventas, color = custom_colors )) %>%
  hc_add_theme(hc_theme_ffx()) %>%
    hc_tooltip(pointFormat = '<b>ventas: </b>{point.y}<br>') %>%
  hc_title(text = 'Ciudades con mas ventas', style = list(fontSize = '25px', fontWeight = 'bold')) 
...
### Ventas Totales Mensuales por año
...
### Aumento de salario por empleado

```{r}
ggplot(historial_salarios, aes(x = Empleado, y = Aumento_Salario, fill = Empleado)) +
  geom_bar(stat = "identity") +
  labs(title = "Aumento de salario por empleado",
       x = "Empleado",
